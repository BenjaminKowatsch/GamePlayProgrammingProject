...
void gpp::sm::StateMachine::setLogging(gep::ILogging* pLogging)
{
    State::setLogging(pLogging);

    // Propagate logging result to the inner states.
    for (auto pState: m_innerStates.values())
    {
        pState->setLogging(m_pLogging);
    }
}
