...
void gpp::Game::initialize()
{
    ...
    g_globalManager.getLogging()->logMessage("=== Finished Initialization Phase ===");
...
void gpp::Game::toggleDisplayMemoryStatistics()
{
    ...
    GEP_ASSERT(pUpdateFramework != nullptr, "No update framework available?!");

    if (m_memoryManagerUpdate.id == -1)
    {
        auto pMemoryManager = g_globalManager.getMemoryManager();
        GEP_ASSERT(pMemoryManager != nullptr, "No memory manager available??");
        m_memoryManagerUpdate = pUpdateFramework->registerUpdateCallback(
            std::bind(&gep::IMemoryManager::update, pMemoryManager, std::placeholders::_1));
    }
    else
    {
