...
logMessage("Initializing animation component test...")
...
Events.Update:registerListener(function(elapsedTime)
	...
	logMessage(bone.x .. "  " .. bone.y .. "	" .. bone.z)
	...
	if InputHandler:wasTriggered(Key.E)then
		logMessage(barbarian.anim:easeOut("walk", 0.9))		
	end
	if InputHandler:wasTriggered(Key.R)then
		logMessage(barbarian.anim:easeIn("walk", 0.9))		
	end
...
logMessage("Finished animation component test initialization.")
