...
gep::AnimationResource* gep::AnimationFileLoader::loadResource( AnimationResource* pInPlace )
{
	...
	auto* container = havokLoader->load(getResourceId());
    GEP_ASSERT(container != nullptr, "Could not load asset! %s", getResourceId());
    ...
    if (container)
    {
        ...
        {
            g_globalManager.getLogging()->logMessage("No skeleton loaded from file %s",getResourceId());
        }
        ...
        {
            g_globalManager.getLogging()->logMessage("No animation loaded from file %s",getResourceId());
        }
        ...
        {
            g_globalManager.getLogging()->logMessage("No binding loaded from file %s",getResourceId());
        }
        ...
        {
             g_globalManager.getLogging()->logMessage("No skin binding loaded from file %s",getResourceId());
        }
