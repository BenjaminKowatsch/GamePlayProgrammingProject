...
{
    IResource* pDummyResource = nullptr;
    if(!m_resourceDummies.tryGet(ptr->getLoader()->getResourceType(), pDummyResource))
    {
        GEP_ASSERT(false, "Resource type not registered yet", ptr->getLoader()->getResourceType());
    }
