...
gep::IResource* gep::FmodSoundLoader::loadResource(IResource* pInst)
{
    if(!m_isIdResolved)
    {
		...
		auto result = pSystem->getFmodHandle().lookupEventID(getResourceId(), &m_id);
        if(result != FMOD_OK)
        {
            std::ostringstream msg;
            msg << "Failed to find event '" << getResourceId() << "' with error: " << FMOD_ErrorString(result);
...
gep::FmodSoundInstanceLoader::FmodSoundInstanceLoader(ResourcePtr<FmodSound> superResource)
    : IResourceLoader(""), m_superResource(superResource)
{
    std::ostringstream id;
	id << m_superResource->getLoader()->getResourceId() << ":" << m_superResource->m_numInstaces;
    m_resourceId = id.str();
...
gep::IResource* gep::FmodSoundInstanceLoader::loadResource(IResource* pInPlace)
{
    ...
    if(result != FMOD_OK)
    {
        ...
        msg << "Failed to create event instance of sound '"
			<< m_superResource->getLoader()->getResourceId()
...
gep::SoundParameter gep::FmodSoundInstance::getParameter(const char* name)
{
    ...
    if(!m_parameter.tryGet(sName, pParam))
    {
        ...
        if(status != FMOD_OK)
        {
			g_globalManager.getLogging()->logWarning("Couldn't find parameter '%s' for fmod event '%s'", name, getLoader()->getSuperResource()->getLoader()->getResourceId());
...
void gep::FmodSoundInstance::update3dAttributes()
{
    ...
    if(result != FMOD_OK)
    {
        g_globalManager.getLogging()->logWarning("Failed to update 3d attributes on sound instance of '%s' with error: %s", m_pLoader->getSuperResource()->getLoader()->getResourceId(), FMOD_ErrorString(result));
    }
}
...
float gep::FmodSoundInstance::getVolume()
{
    ...
    if(result != FMOD_OK)
    {
        g_globalManager.getLogging()->logWarning("Failed to get volume of sound instance of sound '%s' with error: %s", getLoader()->getSuperResource()->getLoader()->getResourceId(), FMOD_ErrorString(result));
    }
    return volume;
}
...
void gep::FmodSoundInstance::play()
{
    ...
    if(result != FMOD_OK)
    {
        g_globalManager.getLogging()->logWarning("Failed to play sound instance of sound '%s' with error: %s", getLoader()->getSuperResource()->getLoader()->getResourceId(), FMOD_ErrorString(result));
    }
}
...
gep::FmodDummySoundLoader::FmodDummySoundLoader() : 
	IResourceLoader("<Dummy FmodSound>")
{
}

gep::FmodDummySoundLoader::FmodDummySoundLoader(const char* resourceId) : 
...
gep::FmodDummySoundInstanceLoader::FmodDummySoundInstanceLoader() : 
	FmodDummySoundLoader("<Dummy FmodSoundInstance>")
{
