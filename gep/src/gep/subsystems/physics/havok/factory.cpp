...
gep::CollisionMeshFileLoader::CollisionMeshFileLoader(const char* path) : 
...
gep::CollisionMesh* gep::CollisionMeshFileLoader::loadResource(CollisionMesh* pInPlace)
{
    ...
    auto* container = havokLoader->load(m_resourceId.c_str());
    GEP_ASSERT(container != nullptr, "Could not load asset! %s", m_resourceId.c_str());
...
void gep::HavokPhysicsFactory::initialize()
{
    ...
    g_globalManager.getResourceManager()->registerResourceType("CollisionMesh");
}
...
namespace
{
    struct RigidBodyRetrievalBehavior
    {
        enum Enum
        {
            RaiseError,
            UseFixedAsFallback,
            UseNullptrAsFallback
        };
    };
}
...
static hkpRigidBody* getRigidBody(ScriptTableWrapper& scriptTable,
                                  const char* szName,
                                  RigidBodyRetrievalBehavior::Enum opt = RigidBodyRetrievalBehavior::RaiseError)
{
    ...
    if (!scriptTable.tryGet(szName, rigidBodyTable))
    {
        switch(opt)
        {
        case RigidBodyRetrievalBehavior::UseFixedAsFallback:
        {
        ...
        case RigidBodyRetrievalBehavior::UseNullptrAsFallback: return nullptr;
        default: break;
        }
...
#include "factory_GenericConstraint.inl"
...
gep::Constraint gep::HavokPhysicsFactory::createConstraint(ScriptTableWrapper& scriptTable)
{
    ...
    case ConstraintType::Generic:
        pConstraint = createGenericConstraint(scriptTable);
        break;
    default:
